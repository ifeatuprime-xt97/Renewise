"""
scripts/generate_mnemonic.py

Generate a new 24-word BIP39 mnemonic for testnet deployment.
...
always amateur amazing among amount amuse analyst anchor ancient anger angle
angry animal ankle announce annual another answer antenna antique anxiety any
apart apology appear apple approve april arch arctic area arena argue arm armed
...
bike bind biology bird birth bitter black blade blame blanket blast bleak
bless blind blood blossom blouse blue blur blush board boat body boil bomb
bone bonus book boost border boring borrow boss bottom bounce box boy bracket
brain brand brass brave bread breeze brick bridge brief bright bring brisk
...
carbon card cargo carpet carry cart case cash casino castle casual cat cattle
catch caught cause caution cave ceiling celery cement census century cereal
...
crazy cream credit creek crew cricket crime crisp critic crop cross crouch
crowd crucial cruel cruise crumble crunch crush cry crystal cube culture cup
cupboard curious current curtain curve cushion custom cute cycle dad damage
damp dance danger daring dash daughter dawn day dead deal debris december
decide decline decorate decrease deer defense defy degree delay deliver demand
...
distance divert divorce dizzy doctor document dog doll dolphin domain donate
...
exist exit exotic expand expect expense expert explain expose express extend
...
firm first fiscal fish fit fitness fix flag flame flash flat flavor flee flesh
flight flip float flock flood floor flower fluid flush fly foam focus fog foil
...
fashion fast fatal fault favor feather federal fee feed female fence festival
few fiber field fifteen fifth fifty fight file fill film final find fine
finger finish fire firm first fish five fix flag flame flash flat flee flesh
flight flip float flock flood floor flower fluid flush fly foam focus fog
foil fold follow food foot force forest forget fork fortune forum forward
found four fox fragile frame frequent fresh friend from front frost fruit
fuel full fun function funny fury furniture future gain galaxy gallery game
gap garage garden garlic garment gas gasp gate gather gauge gaze general
genius genre gentle genuine gesture ghost giant gift giggle ginger girl give
glad glance glare glass glide glimpse globe gloom glory glove glow glue goal
goat goddess gold good goose gorilla gospel gossip govern gown grab grace
grain grant grape grass gravity gray great green grid grief grill grind groove
grip gross ground group grow grunt guard guess guide guilt guitar gun gym
habit hair half hammer hamster hand happy harbor hard harsh harvest hat have
hawk hazard head health heart heavy hedge heel height hello helmet help hen
hero hidden high hill hint hip hire history hobby hockey hold hole holiday
hollow home honey hood hook hope horn horror horse hospital host hotel hour
hover hub huge human humble humor hundred hung hunter hurdle hurry hurt husband
...
inject injury inmate inner innocent input inquiry insane insect inside insight
...
joke journal journey joy judge juice jump jungle junior junk just kangaroo
...
lazy leader leaf learn leave lecture left leg legal lemon level lever liberty
...
manual maple march margin marine market marriage mask mass master match material
math matrix matter maximum maze meadow mean measure meat mechanic medal media
melody melt member memory mention menu mercy merge merit merry mesh message
metal method middle midnight milk million mimic mind minimum minor minute
miracle mirror misery miss mistake mix mixture mobile model modify mom moment
...
natural nature navigation navy near neat neck need negative neglect neighbor
neither neon nerve neutral never news next nice night nine noble noise nominee
...
oven over own owner oxygen oyster ozone pact paddle page pair palace palm panda
panel panic panther paper parade parent park parrot party pass patch path
patient patrol pattern pause pave payment peace peach pear pelican pen penalty
...
pizza place planet plastic plate play please pledge plight plot pluck plug plunge
...
prize problem process produce profit program project promote proof property proper
protect proud prove provide public pudding pull pulp pulse pumpkin punch pupil
...
since sing siren sister situate six size skate sketch ski skill skin skirt skull
slab slam sleep slender slice slide slight slim slogan slot slow slush small
smart smile smoke smooth snack snake snap sniff snow soap soccer social sock
soda soft solar soldier solid solution solve someone song soon sorry sort soul
sound soup sponsor spoon
sport spot spray spread spring spy square squeeze squirrel stable stadium staff
stage stairs stamp stake stand standard start stark state steak steal stem step
stereo stick still sting stock stomach stone stool story stove strategy street
strike strong struggle student stuff stumble style subject submit subway succeed
such sudden suffer sugar suggest suit summer sun sunny sunset super supply
supreme sure surface surge surprise surround survey suspect swallow swap swarm
swear sweet swift swim swing switch sword symbol symptom syrup system table
tackle tag tail talent talk tank tape target task taste tattoo taxi teach team
tell ten tenant tennis tent term test text thank that theme then theory there
they thing this thought three thrive throw thumb thunder ticket tide tiger tilt
timber time tiny tip tired tissue title toast tobacco today toddler toe toilet
token tomato tomorrow tone tongue tonight tool top topic topple torch
tornado tortoise toss total tourist toward tower town toy track trade traffic
tragic trail train trait trash tray treat tree trend trial tribe trick trigger
trim trip trophy trouble truck true trunk trust truth try tube tuition tumble
...
uphold upon upper upset urban urge usage used useful useless usual utility
vacant vacuum vague valid valley valve van vanish vapor various vast vault
vehicle velvet vendor venture venue verb verify version verse very vessel veteran
...
def generate_mnemonic() -> list[str]:
    ...
    # Compute checksum (first 8 bits of SHA256)
    checksum = hashlib.sha256(entropy).digest()[0]
    
    # Combine entropy (256 bits) + checksum (8 bits) = 264 bits
    # For 24 words, we need 256 + 8 = 264 bits, taking first 8 bits of checksum
    ...
    for i in range(24):
        ...
        # For words 0-22: bits come entirely from entropy[0:253]
        # For word 23: last 3 bits from entropy + 8 bits from checksum
        if i < 23:
            # Shift right to get the 11-bit chunk for this word
            shift = 256 - (i + 1) * 11
            index = (entropy_int >> shift) & 0x7FF  # 0x7FF = 11 bits = 2047
        else:
            # Last word: last 3 bits of entropy + 8 bits of checksum
            last_bits = entropy_int & 0x7  # last 3 bits
            index = (last_bits << 8) | checksum
...
def main() -> None:
    # Ensure wordlist has 2048 words
    if len(BIP39_WORDS) != 2048:
        print(f"ERROR: Wordlist has {len(BIP39_WORDS)} words, expected 2048")
        return
    
    words = generate_mnemonic()
    
    print("\n" + "=" * 60)
    print("  NEW 24-WORD MNEMONIC (SAVE THIS SECURELY)")
    print("=" * 60 + "\n")
    print(" ".join(words))
    print("\n" + "=" * 60)
    print("  Fund this wallet via: https://t.me/testgiver_ton_bot")
    print("  Then update DEPLOYER_MNEMONIC in your .env file")
    print("=" * 60 + "\n")
