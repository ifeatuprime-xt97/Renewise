...
async def api_developer_delete_test_keys(
    ...
    """
    Revokes the test key pair for a platform.

    Sets secret_key_test to an empty string (NOT NULL column — cannot be NULL)
    and publishable_key_test to a sentinel value so any cached auth entries
    immediately stop working.  A new pair can be generated by creating the
    platform again, or via the regenerate endpoint.
    """
    telegram_user_id = user["id"]
    await verify_platform_ownership(platform_id, telegram_user_id)
    ...
    from renewise.db.connection import _db as _conn
    async with _conn() as db:
        # Use empty string sentinel — NOT NULL constraint forbids NULL.
        # An empty string will never match a real sk_test_ key so auth fails.
        await db.execute(
            "UPDATE platforms SET secret_key_test = '' WHERE id = $1",
            platform_id
        )
        await db.execute(
            "INSERT INTO platform_audit_log (platform_id, action, actor_telegram_id, details) "
            "VALUES ($1, $2, $3, $4)",
            platform_id, "delete_test_keys", telegram_user_id, "revoked"
        )

    # Invalidate any cached auth entries for old test keys
    from renewise.api.platform import auth_cache
